...
* **CUDA Core:** each subpartition contains a set of ALUs called CUDA Cores that do vector arithmetic. While they may seem similar to the vector processing units (VPUs) found in a TPU, there is a key difference in their programming models. VPUs typically use a Single Instruction, Multiple Data (SIMD) model, whereas CUDA Cores use a Single Instruction, Multiple Threads (SIMT) model.*<d-footnote>GPUs do allow us to instruct each lane to do different work, but if two lanes in the same warp scheduler need to do different operations, the hardware will run multiple passes through the CUDA cores and combine the results. This is called "warp divergence".</d-footnote>

  * From a hardware perspective, SIMT can be thought of as a form of "masked SIMD," but the software API presents a much simpler programming experience. This is because SIMT allows you to program as if you are controlling a single thread, which is much easier than the SIMD model that often requires manual data padding and more complex management of registers and scalar variables. This abstraction simplifies the development of parallel applications.
