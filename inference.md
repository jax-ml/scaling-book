...
One more trick we can employ is speculative decoding<d-cite key="spec1"></d-cite><d-cite key="spec2"></d-cite>. It's a powerful end-to-end latency win, and as model sparsity has increased, it has also become useful as a throughput optimization. As a recap, we usually generate tokens from a large Transformer one by one:

{% include figure.liquid path="assets/img/spec-sampling1.png" class="img-fluid" %}

With speculative decoding, we use a cheaper way (e.g. a smaller model) to generate tokens and then check the result with the model we want to sample from (the target model). This is easiest to understand with *greedy decoding*:

{% include figure.liquid path="assets/img/spec-sampling2.png" class="img-fluid" %}

1. We get some draft tokens, starting from the final token of the previous step. We could do this by sampling greedily from a model trained to match the target model (trained on the same dataset and/or distilled from it), or it could be as simple as using n-grams or matching tokens against a small corpus of text.
2. After getting hold of K draft tokens, we concatenate them with the previous token and score them with the target model. This gives us K "true" logits, one for each draft token, and an extra logit for the token that comes after the entire draft.
3. Since we're decoding greedily, we can just check if each draft token has the highest probability of all possible tokens according to the target model. If one of the tokens is wrong, we take the longest correct prefix and replace the first wrong token with the correct token, then go back to (1). If all the tokens are correct, we can use the last correct logit to sample an extra token before going back to (1).

**Why is this a win?** Scoring K+1 tokens still costs K+1 tokens' worth of target-model FLOPs, but since we're *not compute-bound*, one forward pass over K+1 tokens takes only slightly longer than one over a single token.

For normal autoregressive sampling, tokens/s is just the inverse of the step time. We are still beholden to the theoretical minimum step time from [the section above on latency and throughput](#theoretical-estimates-for-llm-latency-and-throughput). Speculative decoding steps are actually quite a bit slower than normal autoregressive sampling (due to drafting overhead and a slightly slower scoring pass), but because we get between 1 and K+1 tokens per step (depending on how well the draft agrees with the target model), we can get much better tokens/s.

{% include figure.liquid path="assets/img/spec-sampling3.png" class="img-fluid" caption="<b>Figure:</b> this figure shows the per-step latency and speculation success rate for Chinchilla (a 70B model from DeepMind) with a 4B parameter drafter (small model). For XSum (a natural language dataset), it's best to draft about 3-4 tokens ahead, while HumanEval (a coding dataset) is more predictable and benefits from drafting further ahead." %}

Every accepted token becomes more expensive in terms of FLOPs on average (since some will be rejected, and we have to call a draft model), but we wring more FLOPs out of the hardware, and the small model is cheap, so we win overall. We also share KV cache loads across multiple tokens, so speculative decoding can also be a significant throughput win for large KV caches (e.g. longer contexts or less optimized architectures with MHA).

Sparse models need a batch roughly $$E/k$$ times larger to become compute-bound (see [Section 4](../transformers#sparsity-and-mixture-of-experts)), which is hard to reach during decoding, so they often have spare FLOPs for speculative tokens to use for large-scale serving.

Since everything has been checked by the target model, we don't change the sampling distribution at all (though the exact trajectory will differ for non-greedy sampling). We have a lot of flexibility in arriving at a drafter, as long as the higher acceptance rate makes up for the added step time.

Traditionally, speculative decoding relies on having a smaller model with a sampling distribution similar to the target model's (e.g. LLaMA-2 7B for LLaMA-2 70B). A dedicated drafter could also be trained and/or distilled, with architectures optimised for the target model's hardware. With more engineering resources, you can even embed a drafter within the target model, for instance by adding a dedicated drafter head to one of the later layers<d-cite key="eagle"></d-cite><d-cite key="medusa"></d-cite><d-cite key="DeepSeek3"></d-cite>. Because this head has access to the target model's internal representations, it can be much smaller and faster than a standalone drafter while agreeing with the target model more often.

**How does this work for non-greedy decoding?** This is a bit more complicated, but essentially boils down to a Metropolis-Hastings-inspired algorithm. Let $$p$$ be the target model's next-token distribution and $$q$$ be the draft model's, both derived from the logits. For each drafted token $$x$$, we accept it with probability $$\min(1, p(x) / q(x))$$. If we reject it, we throw away the rest of the draft and instead sample a replacement token from the leftover distribution $$\max(0, p - q)$$, renormalized to sum to 1. This correction step is what makes the output distribution *exactly* $$p$$, the same as sampling from the target model directly (up to numeric precision).
...
<p markdown=1 class="takeaway">**Takeaway:** Speculative decoding lets us get more than one token out of each forward pass of the target model. That makes it a large latency win and, when batch size is limited, the model is sparse and/or the KV cache is large, a throughput win too.</p>
