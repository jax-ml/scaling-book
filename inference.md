...
**Why is this a win?** This scheme still requires us to do the FLOPs-equivalent of one forward pass through the target model for K+1 tokens, but because we can batch a bunch of tokens together, we can do all these FLOPs in one forward pass and take advantage of the fact that we're *not compute-bound* to score more tokens for free.

For normal autoregressive sampling, tokens/s is just the inverse of the step time. We are still beholden to the theoretical minimum step time from [the section above on latency and throughput](#theoretical-estimates-for-llm-latency-and-throughput). Speculative decoding step times are actually quite a bit slower than normal autoregressive sampling, but because we get between 1 and K+1 tokens per step (depending on how well the draft agrees with the target model), we can get much better tokens/s.
...
Sparse models need a batch roughly $$E/k$$ times larger to become compute-bound (see [Section 4](../transformers#sparsity-and-mixture-of-experts)), which is hard to reach during decoding, so they often have spare FLOPs for speculative tokens to use for large serving. 
...
Traditionally, speculative decoding relies on having a smaller model with a sampling distribution similar to the target model's (e.g. LLaMA-2 7B for LLaMA-2 70B). But often no one has trained or released such a model, and even when one exists, it may not agree with the target model often enough to pay for its own step time. Instead, it can be helpful to embed a drafter within the main model, for instance by adding a dedicated drafter head to one of the later layers of the base model<d-cite key="eagle"></d-cite><d-cite key="medusa"></d-cite><d-cite key="DeepSeek3"></d-cite>. Because this head has access to the target model's internal representations, it can be much smaller and faster than a standalone drafter while agreeing with the target model more often.
