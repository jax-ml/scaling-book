...
<p markdown=1 class="takeaway">**Takeaway:** for a bfloat16 matmul to be compute-bound on most TPUs, we need our per-replica batch size to be greater than 240.<d-footnote>Remember $B$ counts *tokens*, not sequences — most rooflines depend purely on this token count. For instance, a batch of 512 sequences of 4096 tokens on 2048 GPUs is `512 * 4096 = 2M` total tokens, or a per-replica batch size of 1k tokens.</d-footnote></p>
